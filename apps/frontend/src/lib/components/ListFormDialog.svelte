...
<Dialog.Root bind:open {onOpenChange}>
  ...
  <Dialog.Portal>
    ...
    />
    <Dialog.Content
      class="fixed top-1/2 left-1/2 z-50 w-full max-w-[94%] -translate-x-1/2 -translate-y-1/2 rounded-card border border-border bg-background shadow-popover sm:max-w-[440px]"
    >
      ...
      <form onsubmit={submit} class="flex flex-col gap-4 px-5 py-5">
        {#if !list?.isReadLater}
          <div class="flex flex-col gap-1.5">
            ...
            <input
              ...
              class="h-10 rounded-input border border-border bg-background px-3 text-sm text-foreground placeholder:text-muted-foreground focus-visible:ring-2 focus-visible:ring-foreground/20 focus-visible:outline-hidden"
            />
          </div>
        {/if}
        ...
        <div class="flex flex-col gap-1.5">
          ...
          <textarea
            ...
            class="resize-none rounded-input border border-border bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus-visible:ring-2 focus-visible:ring-foreground/20 focus-visible:outline-hidden"
          ></textarea>
        </div>
        ...
        <div class="flex items-center justify-between gap-3">
          ...
          <Switch.Root
            ...
            class="peer inline-flex h-[28px] min-h-[28px] w-[48px] shrink-0 cursor-pointer items-center rounded-full px-[3px] transition-colors focus-visible:ring-2 focus-visible:ring-foreground focus-visible:ring-offset-2 focus-visible:ring-offset-background focus-visible:outline-hidden data-[state=checked]:bg-foreground data-[state=unchecked]:bg-dark-10"
          >
            <Switch.Thumb
