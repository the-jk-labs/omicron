...
{#if !data.personalized}
  ...
  <section class="mb-10 border-b border-neutral-200 pb-10">
    <h1 class="text-4xl font-bold tracking-tight text-neutral-900 sm:text-5xl">
      Human stories &amp; ideas
    </h1>
    <p class="mt-3 max-w-prose text-lg text-neutral-600">
...
{#snippet feedView(feed: Feed, empty: string)}
  {#if feed.loading && feed.items.length === 0}
    <p class="py-8 text-center text-neutral-400">Loading…</p>
...
{#if data.personalized}
  <Tabs.Root value="for-you" onValueChange={onTabChange} class="rounded-card border-muted bg-background-alt shadow-card mb-4 w-full border p-3">
    <Tabs.List class="rounded-9px bg-dark-10 shadow-mini-inset grid w-full grid-cols-2 gap-1 p-1 text-sm font-semibold leading-[0.01em]">
      <Tabs.Trigger
        value="for-you"
        class="data-[state=active]:shadow-mini inline-flex h-8 items-center justify-center gap-1.5 rounded-[7px] bg-transparent py-2 data-[state=active]:bg-white"
      >
...
{:else}
  <h2 class="mb-2 text-sm font-semibold uppercase tracking-wide text-neutral-500">Latest stories</h2>
  {@render feedView(latest, "No stories on this instance yet.")}
{/if}
