...
for (const post of posts) {
  ...
  if (unexpected.length > 0) {
    skipped++;
    console.log(`• ${post.id} SKIPPED — carries real markup too (${unexpected.join(", ")}); repair it by hand`);
    continue;
  }
  ...
  changed++;
  console.log(`• ${post.id} ${dryRun ? "would be" : ""} repaired (${post.contentHtml.length} → ${fixed.length} chars)`);
