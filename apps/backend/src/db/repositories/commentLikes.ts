...
export async function statsFor(commentIds: string[], viewerId: string | null): Promise<Map<string, LikeStats>> {
  ...
  const rows = await db
    .select({
      ...
      liked: viewerId ? sql<boolean>`bool_or(${commentLikes.userId} = ${viewerId})` : sql<boolean>`false`,
    })
    .from(commentLikes)
