...
import { diversify, MAX_CONSECUTIVE_SAME_AUTHOR, maxPerAuthorPerPage } from "@/lib/feedDiversity.ts";
...
test("maxPerAuthorPerPage: about a fifth of the page, minimum 2", () => {
  expect(maxPerAuthorPerPage(20)).toBe(4);
  expect(maxPerAuthorPerPage(12)).toBe(3);
  expect(maxPerAuthorPerPage(5)).toBe(2);
});

test("diversify: breaks a consecutive same-author run when relief exists", () => {
  ...
  expect(kept.map((r) => r.author)).toEqual(["a", "a", "b"]);
  expect(MAX_CONSECUTIVE_SAME_AUTHOR).toBe(2);
  expect(scanned).toBe(5);
});

test("diversify: share cap limits one author's presence on a page", () => {
  ...
  const { kept } = diversify(rowsOf("a", "a", "b", "a", "a", "c", "a", "a", "d"), 12, (r) => r.author);
  expect(kept.map((r) => r.author)).toEqual(["a", "a", "b", "a", "c", "d"]);
  expect(kept.filter((r) => r.author === "a").length).toBe(maxPerAuthorPerPage(12));
});

test("diversify: relaxes caps rather than returning a short page", () => {
  // Only one author left in the window — a third post in a row beats an
  // empty slot.
  const { kept, scanned } = diversify(rowsOf("a", "a", "a"), 5, (r) => r.author);
  expect(kept.map((r) => r.author)).toEqual(["a", "a", "a"]);
  expect(scanned).toBe(3);
});

test("diversify: reports scanned so the cursor can skip held-back rows", () => {
  const { kept, scanned } = diversify(rowsOf("a", "b", "c", "d"), 2, (r) => r.author);
  expect(kept.map((r) => r.author)).toEqual(["a", "b"]);
  expect(scanned).toBe(2);
});

test("diversify: state carries across windows of one page", () => {
  const { kept: firstKept, state } = diversify(rowsOf("a", "x", "b"), 20, (r) => r.author);
  expect(firstKept.map((r) => r.author)).toEqual(["a", "x", "b"]);
  ...
  expect(kept.map((r) => r.author)).toEqual(["b", "c"]);
});
